<template>
  <!-- 顶部筛选区域 -->
  <SearchComp :goodsCategory="goodsCategory" @searchValue="getSearchGoodsList"></SearchComp>
  ...
  <div class="pagination-container">
    <el-pagination
      ...
      :total="allCategoryGoodsList.length"
...
import SearchComp from '@/views/AppMain/AppProduct/GoodsSearch/components/SearchComp.vue'
import { getGoodsCategoryApi } from '@/api/getTotalCategory'
import { postGoodsSearchApi } from '@/api/getGoodsSearch'
export default {
  data() {
    return {
      // 存放商品列表数据
  ...
  computed: {
    categoryGoodsList() {
      return this.allCategoryGoodsList.slice(
  ...
  methods: {
    ...
    // 子组件触发父组件获取筛选后的商品列表
    async getSearchGoodsList({ search = '', category = '' }) {
      try {
        const res = await postGoodsSearchApi({ search, category })
        this.allCategoryGoodsList = res
      } catch (error) {
        console.log(error)
      }
    }
